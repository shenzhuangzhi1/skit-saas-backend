...
CREATE TRIGGER `trg_callback_route_registry_migration_monotonic` BEFORE UPDATE ON `skit_ad_callback_route_registry_migration` FOR EACH ROW BEGIN IF NEW.`singleton_id` <> OLD.`singleton_id` OR NEW.`last_callback_key_id` < OLD.`last_callback_key_id` OR NEW.`phase_revision` <> OLD.`phase_revision` + 1 OR NOT (NEW.`started_at` <=> OLD.`started_at`) OR OLD.`migration_phase`='ENFORCED' OR NOT (NEW.`migration_phase`=OLD.`migration_phase` OR (OLD.`migration_phase`='DUAL_WRITE' AND NEW.`migration_phase`='BACKFILL') OR (OLD.`migration_phase`='BACKFILL' AND NEW.`migration_phase`='VERIFY') OR (OLD.`migration_phase`='VERIFY' AND NEW.`migration_phase`='SHADOW_READ') OR (OLD.`migration_phase`='SHADOW_READ' AND NEW.`migration_phase`='HASH_FIRST') OR (OLD.`migration_phase`='HASH_FIRST' AND NEW.`migration_phase`='ENFORCED')) OR (OLD.`blocked_at` IS NOT NULL AND NEW.`migration_phase`<>OLD.`migration_phase`) OR (OLD.`verified_at` IS NOT NULL AND NOT (NEW.`expected_row_count` <=> OLD.`expected_row_count` AND NEW.`verified_row_count` <=> OLD.`verified_row_count` AND NEW.`verification_mismatch_count` <=> OLD.`verification_mismatch_count` AND NEW.`verification_hash` <=> OLD.`verification_hash` AND NEW.`verified_at` <=> OLD.`verified_at`)) OR (OLD.`migration_phase`='VERIFY' AND NEW.`migration_phase`='SHADOW_READ' AND NOT (NEW.`expected_row_count`=NEW.`verified_row_count` AND NEW.`verification_mismatch_count`=0 AND NEW.`verification_hash` IS NOT NULL AND NEW.`verified_at` IS NOT NULL)) OR (NEW.`migration_phase`='ENFORCED' AND NEW.`completed_at` IS NULL) OR (NEW.`migration_phase`<>'ENFORCED' AND NEW.`completed_at` IS NOT NULL) OR (OLD.`completed_at` IS NOT NULL AND NOT (NEW.`completed_at` <=> OLD.`completed_at`)) THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'callback registry migration state is monotonic'; END IF; END$$
