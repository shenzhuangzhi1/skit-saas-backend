...
CREATE TABLE IF NOT EXISTS `skit_ad_provider_connection` (
  ...
  `non_terminal_shared_master_slot` varchar(64) GENERATED ALWAYS AS
    (CASE WHEN `provider`='TAKU' AND `account_mode`='SHARED_MASTER' AND `state`<>'RETIRED'
  ...
  CONSTRAINT `ck_provider_connection_mode` CHECK (`account_mode` IN ('SHARED_MASTER','TENANT_OWNED')),
  CONSTRAINT `ck_provider_connection_owner` CHECK
    ((`account_mode`='SHARED_MASTER' AND `owner_tenant_id` IS NULL AND `owner_ad_account_id` IS NULL) OR
     (`account_mode`='TENANT_OWNED' AND `owner_tenant_id` IS NOT NULL AND `owner_ad_account_id` IS NOT NULL)),
  CONSTRAINT `ck_provider_connection_state` CHECK
    (`state` IN ('CONFIGURING','ACTIVE','MIGRATING','BLOCKED','RETIRED'))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Skit 阶段 1 全局 provider connection'$$
...
CREATE TABLE IF NOT EXISTS `skit_ad_provider_callback_route` (
  ...
  `occupied_route_slot` varchar(32) GENERATED ALWAYS AS
    (CASE WHEN `route_slot`<>'INACTIVE' AND `state` NOT IN ('ABANDONED','RETIRED')
  ...
  CONSTRAINT `ck_provider_callback_route_purpose` CHECK (`purpose` IN ('GATE_TEST','PRODUCTION')),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Skit 阶段 1 全局 provider callback route'$$

CREATE TABLE IF NOT EXISTS `skit_ad_callback_route_registry` (
  ...
  CONSTRAINT `fk_callback_route_registry_provider_route` FOREIGN KEY (`provider_callback_route_id`)
  ...
  CONSTRAINT `ck_callback_route_registry_owner_xor` CHECK
    ((`owner_type`='PROVIDER_CALLBACK_ROUTE' AND `provider_callback_route_id` IS NOT NULL
      AND `tenant_callback_key_id` IS NULL) OR
     (`owner_type`='TENANT_CALLBACK_KEY' AND `provider_callback_route_id` IS NULL
      AND `tenant_callback_key_id` IS NOT NULL))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Skit 阶段 1 全局 callback route registry'$$

CREATE TABLE IF NOT EXISTS `skit_ad_callback_route_registry_migration` (
  ...
  `last_error_hash` binary(32) DEFAULT NULL, `created_at` datetime NOT NULL, `updated_at` datetime NOT NULL,
  ...
  CONSTRAINT `ck_callback_route_registry_migration_time` CHECK
    ((`migration_state`='PENDING' AND `started_at` IS NULL AND `completed_at` IS NULL) OR
     (`migration_state` IN ('RUNNING','BLOCKED') AND `started_at` IS NOT NULL AND `completed_at` IS NULL) OR
     (`migration_state`='COMPLETED' AND `started_at` IS NOT NULL AND `completed_at` IS NOT NULL))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Skit 阶段 1 全局 registry migration state'$$
...
CREATE TABLE IF NOT EXISTS `skit_provider_impression_inbox` (
  ...
  `canonical_attempt_id` bigint DEFAULT NULL, `capture_status` varchar(16) NOT NULL DEFAULT 'CAPTURED',
  ...
  CONSTRAINT `ck_provider_impression_inbox_time` CHECK (`last_received_at`>=`first_received_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Skit 阶段 1 全局 provider impression inbox'$$
...
CREATE TABLE IF NOT EXISTS `skit_provider_callback_attempt` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_provider_callback_attempt_correlation` (`correlation_id`),
  ...
  CONSTRAINT `ck_provider_callback_attempt_expiry` CHECK (`payload_expires_at`>`received_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Skit 阶段 1 全局 provider callback attempt'$$

DROP PROCEDURE IF EXISTS `skit_add_provider_canonical_attempt_fk`$$
CREATE PROCEDURE `skit_add_provider_canonical_attempt_fk`()
BEGIN
  DECLARE `existing_count` int DEFAULT 0;
  SELECT COUNT(*) INTO `existing_count` FROM information_schema.TABLE_CONSTRAINTS
    WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='skit_provider_impression_inbox'
      AND CONSTRAINT_NAME='fk_provider_impression_inbox_canonical_attempt';
  IF `existing_count`=0 THEN
    ALTER TABLE `skit_provider_impression_inbox`
      ADD CONSTRAINT `fk_provider_impression_inbox_canonical_attempt`
      FOREIGN KEY (`canonical_attempt_id`) REFERENCES `skit_provider_callback_attempt` (`id`)
      ON UPDATE RESTRICT ON DELETE RESTRICT;
  END IF;
END$$
CALL `skit_add_provider_canonical_attempt_fk`()$$
DROP PROCEDURE `skit_add_provider_canonical_attempt_fk`$$
...
CREATE TABLE IF NOT EXISTS `skit_platform_provider_command_audit` (
  ...
  KEY `idx_platform_provider_command_audit_connection` (`provider_connection_id`,`occurred_at`,`id`),
  ...
  CONSTRAINT `fk_platform_provider_command_audit_route` FOREIGN KEY (`provider_callback_route_id`)
    REFERENCES `skit_ad_provider_callback_route` (`id`) ON UPDATE RESTRICT ON DELETE RESTRICT,
  CONSTRAINT `fk_platform_provider_command_audit_registry` FOREIGN KEY (`callback_route_registry_id`)
    REFERENCES `skit_ad_callback_route_registry` (`id`) ON UPDATE RESTRICT ON DELETE RESTRICT,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Skit 阶段 1 全局平台 provider 命令审计'$$
...
DROP TRIGGER IF EXISTS `trg_provider_callback_route_purpose_immutable`$$
CREATE TRIGGER `trg_provider_callback_route_purpose_immutable`
BEFORE UPDATE ON `skit_ad_provider_callback_route` FOR EACH ROW
BEGIN
  IF NOT (NEW.`purpose` <=> OLD.`purpose`) THEN
...
BEFORE UPDATE ON `skit_ad_callback_route_registry_migration` FOR EACH ROW
BEGIN
  IF NEW.`singleton_id` <> OLD.`singleton_id`
    ...
    OR (OLD.`completed_at` IS NOT NULL AND NOT (NEW.`completed_at` <=> OLD.`completed_at`)) THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'callback registry migration state is monotonic';
  END IF;
END$$
DROP TRIGGER IF EXISTS `trg_callback_route_registry_migration_no_delete`$$
CREATE TRIGGER `trg_callback_route_registry_migration_no_delete`
BEFORE DELETE ON `skit_ad_callback_route_registry_migration` FOR EACH ROW
BEGIN
  SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'callback registry migration state cannot be deleted';
END$$
DROP TRIGGER IF EXISTS `trg_platform_provider_command_audit_immutable`$$
CREATE TRIGGER `trg_platform_provider_command_audit_immutable`
BEFORE UPDATE ON `skit_platform_provider_command_audit` FOR EACH ROW
BEGIN
  SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'platform provider command audit is append only';
END$$
DROP TRIGGER IF EXISTS `trg_platform_provider_command_audit_no_delete`$$
CREATE TRIGGER `trg_platform_provider_command_audit_no_delete`
BEFORE DELETE ON `skit_platform_provider_command_audit` FOR EACH ROW
BEGIN
  SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'platform provider command audit cannot be deleted';
END$$
DELIMITER ;
