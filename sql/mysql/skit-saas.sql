...
CREATE TRIGGER `trg_provider_impression_inbox_monotonic` BEFORE UPDATE ON `skit_provider_impression_inbox` FOR EACH ROW BEGIN IF NOT (NEW.`provider_connection_id` <=> OLD.`provider_connection_id` AND NEW.`dedupe_scheme` <=> OLD.`dedupe_scheme` AND NEW.`dedupe_key_hash` <=> OLD.`dedupe_key_hash` AND NEW.`provider_request_id_lexical` <=> OLD.`provider_request_id_lexical` AND NEW.`adsource_id_lexical` <=> OLD.`adsource_id_lexical` AND NEW.`material_integrity_hash` <=> OLD.`material_integrity_hash` AND NEW.`authentication_level` <=> OLD.`authentication_level` AND NEW.`first_received_at` <=> OLD.`first_received_at`) OR NOT ((NEW.`canonical_attempt_id` <=> OLD.`canonical_attempt_id`) OR (OLD.`canonical_attempt_id` IS NULL AND NEW.`canonical_attempt_id` IS NOT NULL)) OR NEW.`delivery_count` < OLD.`delivery_count` OR NEW.`processing_attempt_count` < OLD.`processing_attempt_count` OR NEW.`last_received_at` < OLD.`last_received_at` OR NOT (((NEW.`integrity_status` <=> OLD.`integrity_status`) AND NEW.`integrity_revision`=OLD.`integrity_revision` AND (NEW.`integrity_conflict_at` <=> OLD.`integrity_conflict_at`)) OR (OLD.`integrity_status`='CANONICAL' AND NEW.`integrity_status`='PAYLOAD_CONFLICT' AND NEW.`integrity_revision`=OLD.`integrity_revision`+1 AND NEW.`delivery_count`>OLD.`delivery_count` AND OLD.`integrity_conflict_at` IS NULL AND NEW.`integrity_conflict_at` IS NOT NULL) OR (OLD.`integrity_status`='PAYLOAD_CONFLICT' AND NEW.`integrity_status`='PAYLOAD_CONFLICT' AND (NEW.`integrity_revision`=OLD.`integrity_revision` OR (NEW.`integrity_revision`=OLD.`integrity_revision`+1 AND NEW.`delivery_count`>OLD.`delivery_count`)) AND NEW.`integrity_conflict_at` <=> OLD.`integrity_conflict_at`)) OR (NEW.`dedupe_scheme`='FALLBACK_WIRE_V1' AND NEW.`processing_status`<>'QUARANTINED') OR (OLD.`processing_status` IN ('SUCCEEDED','QUARANTINED','DEAD_LETTER') AND NEW.`processing_status`<>OLD.`processing_status`) OR NOT (NEW.`processing_status`=OLD.`processing_status` OR (OLD.`processing_status`='PENDING' AND NEW.`processing_status` IN ('PROCESSING','QUARANTINED')) OR (OLD.`processing_status`='PROCESSING' AND NEW.`processing_status` IN ('RETRY_WAIT','SUCCEEDED','QUARANTINED','DEAD_LETTER')) OR (OLD.`processing_status`='RETRY_WAIT' AND NEW.`processing_status`='PROCESSING')) THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'provider impression inbox is monotonic'; END IF; END$$
DROP TRIGGER IF EXISTS `trg_provider_callback_attempt_immutable`$$
CREATE TRIGGER `trg_provider_callback_attempt_immutable` BEFORE UPDATE ON `skit_provider_callback_attempt` FOR EACH ROW BEGIN IF NOT (NEW.`id` <=> OLD.`id` AND NEW.`correlation_id` <=> OLD.`correlation_id` AND NEW.`provider_connection_id` <=> OLD.`provider_connection_id` AND NEW.`inbox_id` <=> OLD.`inbox_id` AND NEW.`wire_payload_hash` <=> OLD.`wire_payload_hash` AND NEW.`material_integrity_hash` <=> OLD.`material_integrity_hash` AND NEW.`delivery_integrity_status` <=> OLD.`delivery_integrity_status` AND NEW.`response_decision` <=> OLD.`response_decision` AND NEW.`wire_size_bytes` <=> OLD.`wire_size_bytes` AND NEW.`parameter_count` <=> OLD.`parameter_count` AND NEW.`remote_address_hash` <=> OLD.`remote_address_hash` AND NEW.`user_agent_hash` <=> OLD.`user_agent_hash` AND NEW.`request_header_fingerprint` <=> OLD.`request_header_fingerprint` AND NEW.`trace_id` <=> OLD.`trace_id` AND NEW.`received_at` <=> OLD.`received_at`) OR NOT ((NEW.`payload_ciphertext` <=> OLD.`payload_ciphertext` AND NEW.`payload_nonce` <=> OLD.`payload_nonce` AND NEW.`payload_key_id` <=> OLD.`payload_key_id` AND NEW.`payload_purpose` <=> OLD.`payload_purpose` AND NEW.`payload_envelope_version` <=> OLD.`payload_envelope_version` AND NEW.`payload_expires_at` <=> OLD.`payload_expires_at` AND NEW.`payload_purged_at` <=> OLD.`payload_purged_at`) OR (OLD.`payload_ciphertext` IS NOT NULL AND OLD.`payload_nonce` IS NOT NULL AND OLD.`payload_key_id` IS NOT NULL AND OLD.`payload_purpose` IS NOT NULL AND OLD.`payload_envelope_version` IS NOT NULL AND OLD.`payload_expires_at` IS NOT NULL AND OLD.`payload_purged_at` IS NULL AND NEW.`payload_ciphertext` IS NULL AND NEW.`payload_nonce` IS NULL AND NEW.`payload_key_id` IS NULL AND NEW.`payload_purpose` IS NULL AND NEW.`payload_envelope_version` IS NULL AND NEW.`payload_expires_at` IS NULL AND NEW.`payload_purged_at` IS NOT NULL)) THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'provider callback attempt is immutable except purge'; END IF; END$$
