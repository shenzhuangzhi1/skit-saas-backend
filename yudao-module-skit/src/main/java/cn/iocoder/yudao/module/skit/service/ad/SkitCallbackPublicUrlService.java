...
public class SkitCallbackPublicUrlService {
  ...
  private static final String REWARD_QUERY =
      "?user_id={user_id}&trans_id={trans_id}"
          ...
          + "&sign={sign}&ilrd={ilrd}";
  private static final String IMPRESSION_QUERY =
  ...
  private static final String PROVIDER_IMPRESSION_TEMPLATE =
      PROVIDER_IMPRESSION_PATH + "{callback_key}" + PROVIDER_IMPRESSION_SUFFIX + IMPRESSION_QUERY;
  ...
  public char[] providerImpressionCallbackUrl(char[] callbackKey) {
    ...
    try {
      ...
      System.arraycopy(callbackKey, 0, result, offset, callbackKey.length);
      offset += callbackKey.length;
  ...
  public byte[] providerImpressionContractFingerprint(byte[] callbackKeyHash) {
    ...
    try {
      return providerContractFingerprint(
          callbackKeyHash,
          origin,
          PROVIDER_IMPRESSION_PATH_VERSION,
          PROVIDER_IMPRESSION_TEMPLATE_VERSION,
          template);
    } finally {
  ...
  /**
   * Key-independent deployment contract used before a callback key exists. The persisted route
   * fingerprint above remains key-bound; this fingerprint only proves the accepted public origin
   * and the versioned provider template that deployment evidence was collected against.
   */
  public byte[] providerImpressionDeploymentContractFingerprint() {
    char[] origin = publicBaseUrl.toCharArray();
    char[] template = PROVIDER_IMPRESSION_TEMPLATE.toCharArray();
    try {
      return providerDeploymentContractFingerprint(
          origin, PROVIDER_IMPRESSION_PATH_VERSION, PROVIDER_IMPRESSION_TEMPLATE_VERSION, template);
    } finally {
      Arrays.fill(origin, '\0');
      Arrays.fill(template, '\0');
    }
  ...
  static byte[] providerContractFingerprint(
  ...
  static byte[] providerDeploymentContractFingerprint(
      char[] origin, int pathVersion, int templateVersion, char[] template) {
    if (origin == null
        || origin.length == 0
        || template == null
        || template.length == 0
        || pathVersion <= 0
        || templateVersion <= 0) {
      throw new IllegalArgumentException("Callback deployment contract is invalid");
    }
    try {
      MessageDigest digest = MessageDigest.getInstance("SHA-256");
