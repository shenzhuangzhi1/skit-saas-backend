...
final class SkitProviderImpressionPhase1Schema {
    ...
    static List<Step> steps() {
        ...
        steps.add(Step.trigger("skit_provider_callback_attempt",
                ...
                "BEGIN IF NOT (NEW.`id` <=> OLD.`id` AND "
                        ...
                        + "NEW.`received_at` <=> OLD.`received_at`) OR NOT (("
                        + "NEW.`payload_ciphertext` <=> OLD.`payload_ciphertext` AND "
                        ...
                        + "NEW.`payload_purged_at` <=> OLD.`payload_purged_at`) OR ("
                        + "OLD.`payload_ciphertext` IS NOT NULL AND OLD.`payload_nonce` IS NOT NULL "
                        + "AND OLD.`payload_key_id` IS NOT NULL AND OLD.`payload_purpose` IS NOT NULL "
                        ...
                        + "THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'provider callback attempt is immutable except purge'; "
                        + "END IF; END"));
        steps.add(Step.trigger("skit_provider_callback_attempt",
    ...
    private static String providerImpressionInboxTable() {
        return "CREATE TABLE IF NOT EXISTS `skit_provider_impression_inbox` ("
                ...
                + "PRIMARY KEY (`id`),"
                + "UNIQUE KEY `uk_provider_impression_inbox_connection_id` (`provider_connection_id`,`id`),"
    ...
    private static String providerCallbackAttemptTable() {
        return "CREATE TABLE IF NOT EXISTS `skit_provider_callback_attempt` ("
                ...
                + "(`provider_connection_id`,`inbox_id`,`received_at`,`id`),"
                ...
                + "FOREIGN KEY (`provider_connection_id`,`inbox_id`) "
                + "REFERENCES `skit_provider_impression_inbox` (`provider_connection_id`,`id`) "
                + "ON UPDATE RESTRICT ON DELETE RESTRICT,"
