...
final class SkitProviderImpressionPhase1Schema {
    ...
    static List<Step> steps() {
        ...
        steps.add(Step.trigger("skit_ad_callback_route_registry_migration",
                ...
                callbackRouteRegistryMigrationMonotonicAction()));
        steps.add(Step.trigger("skit_ad_callback_route_registry_migration",
    ...
    private static String callbackRouteRegistryMigrationTable() {
        return "CREATE TABLE IF NOT EXISTS `skit_ad_callback_route_registry_migration` ("
                ...
                + "`verification_snapshot_epoch` bigint DEFAULT NULL,"
                ...
                + "CONSTRAINT `ck_callback_route_registry_migration_revision` CHECK "
                + "(`phase_revision`>=0 AND `credential_mutation_epoch`>=0 AND `verification_run_id`>=0),"
                ...
                + "AND `verification_actual_progress_count`<=`verification_expected_progress_count` "
                + "AND `verification_progress_mismatch_count`>=0 "
                + "AND `verification_progress_mismatch_count`<=`verification_expected_progress_count`),"
                + "CONSTRAINT `ck_callback_route_registry_migration_verification_progress` CHECK "
    ...
    private static String callbackRouteRegistryMigrationMonotonicAction() {
        String progressChanged = "(NOT (NEW.`verification_cursor_callback_key_id` <=> "
                + "OLD.`verification_cursor_callback_key_id`) OR NOT ("
                + "NEW.`verification_expected_progress_count` <=> "
                + "OLD.`verification_expected_progress_count`) OR NOT ("
                + "NEW.`verification_actual_progress_count` <=> "
                + "OLD.`verification_actual_progress_count`) OR NOT ("
                + "NEW.`verification_progress_mismatch_count` <=> "
                + "OLD.`verification_progress_mismatch_count`) OR NOT ("
                + "NEW.`verification_expected_rolling_hash` <=> "
                + "OLD.`verification_expected_rolling_hash`) OR NOT ("
                + "NEW.`verification_actual_rolling_hash` <=> "
                + "OLD.`verification_actual_rolling_hash`))";
        String finalChanged = "(NOT (NEW.`expected_row_count` <=> OLD.`expected_row_count`) OR "
                + "NOT (NEW.`verified_row_count` <=> OLD.`verified_row_count`) OR "
                + "NOT (NEW.`verification_mismatch_count` <=> OLD.`verification_mismatch_count`) OR "
                + "NOT (NEW.`verification_hash` <=> OLD.`verification_hash`) OR "
                + "NOT (NEW.`verified_at` <=> OLD.`verified_at`))";
        String atomicCutover = "(OLD.`migration_phase`='VERIFY' "
                + "AND NEW.`migration_phase`='SHADOW_READ' "
                + "AND OLD.`verified_at` IS NULL AND NEW.`verified_at` IS NOT NULL "
                + "AND NEW.`verification_snapshot_epoch`=NEW.`credential_mutation_epoch` "
                + "AND NEW.`verification_expected_progress_count`="
                + "NEW.`verification_actual_progress_count` "
                + "AND NEW.`verification_progress_mismatch_count`=0 "
                + "AND NEW.`verification_expected_rolling_hash`="
                + "NEW.`verification_actual_rolling_hash` "
                + "AND NEW.`expected_row_count`=NEW.`verification_expected_progress_count` "
                + "AND NEW.`verified_row_count`=NEW.`verification_actual_progress_count` "
                + "AND NEW.`verification_mismatch_count`=0 "
                + "AND NEW.`verification_hash` IS NOT NULL AND NEW.`blocked_at` IS NULL)";
        return "BEGIN IF NEW.`singleton_id` <> OLD.`singleton_id` "
                ...
                + "OR NEW.`credential_mutation_epoch` < OLD.`credential_mutation_epoch` "
                + "OR NEW.`credential_mutation_epoch` > OLD.`credential_mutation_epoch` + 1 "
                + "OR (NEW.`credential_mutation_epoch`<>OLD.`credential_mutation_epoch` AND NOT ("
                + "NEW.`migration_phase`=OLD.`migration_phase` "
                + "AND OLD.`migration_phase` IN ('DUAL_WRITE','BACKFILL','VERIFY') "
                + "AND NEW.`last_callback_key_id`=OLD.`last_callback_key_id` "
                + "AND NEW.`last_batch_size`=OLD.`last_batch_size` "
                + "AND NEW.`verification_run_id`=OLD.`verification_run_id` "
                + "AND NOT " + progressChanged + " AND NOT " + finalChanged + " "
                + "AND NEW.`blocked_at` <=> OLD.`blocked_at` "
                + "AND NEW.`blocked_reason_hash` <=> OLD.`blocked_reason_hash` "
                + "AND NEW.`completed_at` <=> OLD.`completed_at`)) "
                + "OR NEW.`verification_run_id` < OLD.`verification_run_id` "
                + "OR NEW.`verification_run_id` > OLD.`verification_run_id` + 1 "
                + "OR (NEW.`verification_run_id`=OLD.`verification_run_id`+1 AND NOT ("
                + "((OLD.`migration_phase`='BACKFILL' AND NEW.`migration_phase`='VERIFY') OR "
                + "(OLD.`migration_phase`='VERIFY' AND NEW.`migration_phase`='VERIFY' "
                + "AND NOT (OLD.`verification_snapshot_epoch` <=> OLD.`credential_mutation_epoch`))) "
                + "AND NEW.`verification_snapshot_epoch`=NEW.`credential_mutation_epoch` "
                + "AND NEW.`verification_cursor_callback_key_id`=0 "
                + "AND NEW.`verification_expected_progress_count`=0 "
                + "AND NEW.`verification_actual_progress_count`=0 "
                + "AND NEW.`verification_progress_mismatch_count`=0 "
                + "AND NEW.`verification_expected_rolling_hash` IS NOT NULL "
                + "AND NEW.`verification_expected_rolling_hash`="
                + "NEW.`verification_actual_rolling_hash` "
                + "AND NEW.`verified_at` IS NULL AND NEW.`blocked_at` IS NULL)) "
                + "OR (NEW.`verification_run_id`=OLD.`verification_run_id` AND ("
                + "NOT (NEW.`verification_snapshot_epoch` <=> OLD.`verification_snapshot_epoch`) OR "
                + "NEW.`verification_cursor_callback_key_id`<OLD.`verification_cursor_callback_key_id` OR "
                + "NEW.`verification_expected_progress_count`<OLD.`verification_expected_progress_count` OR "
                + "NEW.`verification_actual_progress_count`<OLD.`verification_actual_progress_count` OR "
                + "NEW.`verification_progress_mismatch_count`<OLD.`verification_progress_mismatch_count` OR "
                + "(" + progressChanged + " AND NOT ("
                + "OLD.`migration_phase`='VERIFY' AND NEW.`migration_phase`='VERIFY' "
                + "AND NEW.`verification_snapshot_epoch`=NEW.`credential_mutation_epoch` "
                + "AND OLD.`verified_at` IS NULL AND OLD.`blocked_at` IS NULL "
                + "AND NEW.`verification_cursor_callback_key_id`>"
                + "OLD.`verification_cursor_callback_key_id` "
                + "AND NEW.`verification_expected_progress_count`>"
                + "OLD.`verification_expected_progress_count` "
                + "AND NEW.`verification_actual_progress_count`>="
                + "OLD.`verification_actual_progress_count` "
                + "AND NEW.`verification_progress_mismatch_count`>="
                + "OLD.`verification_progress_mismatch_count` "
                + "AND NEW.`verification_expected_rolling_hash` IS NOT NULL "
                + "AND NEW.`verification_actual_rolling_hash` IS NOT NULL)))) "
                + "OR (" + finalChanged + " AND NOT " + atomicCutover + ") "
                + "OR (OLD.`verified_at` IS NOT NULL AND " + finalChanged + ") "
                + "OR (OLD.`migration_phase`='VERIFY' AND NEW.`migration_phase`='SHADOW_READ' "
                + "AND NOT " + atomicCutover + ") "
                + "OR (OLD.`blocked_at` IS NOT NULL AND NOT ("
                ...
                + "OR (NEW.`blocked_at` IS NOT NULL AND OLD.`blocked_at` IS NULL AND NOT ("
                + "NEW.`migration_phase`=OLD.`migration_phase` "
                + "AND OLD.`migration_phase` IN ('BACKFILL','VERIFY'))) "
                ...
                + "'callback registry migration state is monotonic'; END IF; END";
    }
