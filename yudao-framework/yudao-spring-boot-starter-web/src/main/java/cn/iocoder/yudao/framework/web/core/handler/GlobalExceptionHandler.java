...
public class GlobalExceptionHandler {
  ...
  public CommonResult<?> noHandlerFoundExceptionHandler(
    ...
    log.warn(
        "[noHandlerFoundExceptionHandler][url({}) method({}) exception({})]",
        safeUrl,
        ex.getHttpMethod(),
  ...
  public CommonResult<?> accessDeniedExceptionHandler(
    ...
    log.warn(
        "[accessDeniedExceptionHandler][userId({}) 无法访问 url({}) exception({})]",
        WebFrameworkUtils.getLoginUserId(req),
        ApiRequestUrlResolver.resolve(req),
        safeExceptionType(ex));
   ...
   *
   * <p>例如说，缓存加载报错，可见 <a href="https://t.zsxq.com/UszdH">https://t.zsxq.com/UszdH</a>
   */
  @ExceptionHandler(value = UncheckedExecutionException.class)
  public CommonResult<?> uncheckedExecutionExceptionHandler(
  ...
  public CommonResult<?> defaultExceptionHandler(HttpServletRequest req, Throwable ex) {
    ...
    // 情况二：处理异常
    log.error(
        "[defaultExceptionHandler][url({}) exception({}) rootCause({})]",
        ApiRequestUrlResolver.resolve(req),
        safeExceptionType(ex),
        safeRootCauseType(ex));
    // 插入异常日志
  ...
  private void createExceptionLog(HttpServletRequest req, Throwable e) {
    ...
    try {
      ...
      log.error(
          "[createExceptionLog][url({}) exception({}) rootCause({}) 发生异常]",
          safeUrl,
          safeExceptionType(th),
    ...
    if (suppressParameters) {
      ...
      Map<String, Object> requestParams =
          MapUtil.<String, Object>builder()
              ...
              .put("body", sanitizeJson(ServletUtils.getBody(request), null))
              .build();
  ...
  private static void logSafeWarning(String handler, Throwable throwable) {
    log.warn(
        "[{}][exception({}) rootCause({})]",
        handler,
        safeExceptionType(throwable),
        safeRootCauseType(throwable));
  }
