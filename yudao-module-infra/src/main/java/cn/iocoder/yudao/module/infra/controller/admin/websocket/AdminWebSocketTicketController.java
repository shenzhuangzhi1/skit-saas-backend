...
public class AdminWebSocketTicketController {
    ...
    public ResponseEntity<CommonResult<AdminWebSocketTicketRespVO>> issue(
            @AuthenticationPrincipal LoginUser loginUser) {
        if (loginUser == null || !UserTypeEnum.ADMIN.getValue().equals(loginUser.getUserType())) {
            throw new AccessDeniedException("Only authenticated administrators may issue admin WebSocket tickets");
        }
        WebSocketTicket ticket = ticketService.issue(loginUser);
        CommonResult<AdminWebSocketTicketRespVO> body = success(
                new AdminWebSocketTicketRespVO(ticket.getTicket(), ticket.getExpiresInSeconds()));
